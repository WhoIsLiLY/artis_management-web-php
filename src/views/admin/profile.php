<?php
    ...
    if(isset($_GET['id'])){
        $data = $userController->getAdminById($middleware->getIdFromToken($_COOKIE["token"]));
    }else{
        header("Location:/src/views/admin/workspace.php");
    }
    if(isset($_POST['submit'])){
        //print_r($_POST['data']);
        $userController->updateAdminAuthById($_GET['id'], $_POST["data"]);
        
        header("Location: /src/views/admin/workspace.php?id=".$_GET['id']);
    }
?>
