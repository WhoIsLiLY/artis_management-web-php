<?php
    ...
    if(isset($_GET['id'])){
        $data = $userController->getArtisById($middleware->getIdFromToken($_COOKIE["token"]));
    }else{
        header("Location:/src/views/artis/workspace.php");
    }
    if(isset($_POST['submit'])){
        $userController->updateArtisAuthById($_GET['id'], $_POST["data"]);
        
        header("Location: /src/views/artis/workspace.php?id=".$_GET['id']);
    }
?>
...
<body class="h-screen bg-gradient-to-br from-blue-600 to-purple-700">
    ...
    <div class="container mt-5">
        ...
        <form action="" method="post">
            <div class="mb-3">
                <label for="nama" class="form-label">Admin Name: </label>
